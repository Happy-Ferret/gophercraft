...
type Warden struct {
	InputCrypto, OutputCrypto *arc4.Cipher
	Module                    *WardenModule
...
func UnmarshalWardenServerChecks(input []byte, xorByte uint8) ([]PCheck, error) {
	...
	glogger.Println("Driver headers: ", spew.Sdump(buf.Bytes()))

	if e.ReadByte() == (packet.TIMING_CHECK ^ xorByte) {
		glogger.Warnln("TIMING_CHECK detected")
	}
