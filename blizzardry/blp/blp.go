...
/* package blp decodes Blizzard's BLP images.
...
func (b *BLP) getPixels(mipmap int) []byte {
	...
	b.buf.RPos(int(b.MipmapOffset[mipmap]))
...
func (b *BLP) GetUncompressed() []byte {
	...
	d.WriteBytes(make([]byte, (b.ScaledLength*4)-d.Len()))
	return d.Encode()
}
