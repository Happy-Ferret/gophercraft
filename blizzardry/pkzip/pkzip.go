/* package pkzip implements a decoder for the PKWARE compression algorithm, as used in
   World of Warcraft's MPQ decoder.

   Sloppily translated from what I assume to be a automatically decompiled version of PKWARE's proprietary algorithm.
...
var (
	LIBMPQ_PKZIP_CMP_BINARY       uint32 = 0 /* binary compression. */
	LIBMPQ_PKZIP_CMP_ASCII        uint32 = 1 /* ascii compression. */
	LIBMPQ_PKZIP_CMP_NO_ERROR     uint32 = 0
	LIBMPQ_PKZIP_CMP_INV_DICTSIZE uint32 = 1
	LIBMPQ_PKZIP_CMP_INV_MODE     uint32 = 2
	LIBMPQ_PKZIP_CMP_BAD_DATA     uint32 = 3
	LIBMPQ_PKZIP_CMP_ABORT        uint32 = 4
)
...
var (
	...
	errDictSize    = fmt.Errorf("LIBMPQ_PKZIP_CMP_INV_DICTSIZE")
...
func do_decompress_pkzip(param *info) error {
	...
	if p.cmp_type != LIBMPQ_PKZIP_CMP_BINARY {
		if p.cmp_type != LIBMPQ_PKZIP_CMP_ASCII {
