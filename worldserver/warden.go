...
func (s *Session) InitWarden() {
	...
	w.InputCrypto = arc4.ARC4(inputKey)
	w.OutputCrypto = arc4.ARC4(outputKey)
	...
	pkt.Write(d)
	s.WriteCrypt(pkt.Encode())
}

func (s *Session) TransferWardenModule() {
	glogger.Warnln("Beginning transfer of Warden module.")
...
func (s *Session) RequestHash() {
	p := packet.NewEtcBuffer(nil)
	p.WriteByte(packet.WARDEN_SMSG_HASH_REQUEST)
	p.WriteBytes(s.Warden.Module.Seed)
	d := p.Encode()
	s.Warden.OutputCrypto.Encrypt(d)
	wp := packet.NewWorldPacket(packet.SMSG_WARDEN_DATA)
	wp.Write(d)
	s.WriteCrypt(wp.Encode())
	glogger.Warnln("Hash requested.")
}

func (s *Session) HandleHashResult(data *packet.EtcBuffer) {
	data.RPos(0)
	data.ReadByte()
	h := data.ReadBytes(20)
	if !bytes.Equal(h, s.Warden.Module.ClientKeySeedHash) {
		glogger.Warnln("Client failed Warden hash reply.")
		return
	}

	glogger.Warnln("Correct Warden module verified")

	s.Warden.InputCrypto = arc4.ARC4(s.Warden.Module.ClientKeySeed)
	s.Warden.OutputCrypto = arc4.ARC4(s.Warden.Module.ServerKeySeed)
}
...
func (s *Session) HandleCheck(data *packet.EtcBuffer) {
	...
	for _, chk := range warden.GetChecks() {
		...
		case packet.LUA_STR_CHECK:
			...
			if luaResult != 0 {
				glogger.Warnln("Client failed LUA_STR_CHECK, not zero")
				s.FailCheck()
				return
			}
			...
			glogger.Warnln("Lua string: ", luaStr)
			warden.CheckResults[cid] = []byte(luaStr)
		case packet.MPQ_CHECK:
			mpqResult := data.ReadByte()
			if mpqResult != 0 {
				glogger.Warnln("Client failed MPQ_CHECK, not zero")
				s.FailCheck()
				return
			}
			mpqHash := data.ReadBytes(20)
			glogger.Warnln("MPQ_CHECK", cid, "result", mpqHash)
			warden.CheckResults[cid] = mpqHash
		default:
			glogger.Warnln("Unhandled opcode", chk.Type)
		}

		glogger.Warnln("Check", cid, "passed.")
	}

	glogger.Warnln("Client passed all checks!")

	// glogger.Warnln("Client check data: ", data.Len(), spew.Sdump(data.ReadBytes(128)))
...
func printMap(input map[uint16][]byte) {
	...
	glogger.Warnln(srcBuf)
}

func (s *Session) WardenResponse(data []byte) {
	if s.Warden == nil {
		glogger.Warnln("Received Warden data before Warden initialized")
		return
	}
	...
	p := packet.NewEtcBuffer(dat)
	if p.Len() == 0 {
		glogger.Warnln("User sent too short packet")
	}
	glogger.Warnln("Leng??", p.Len())
	opcode := p.ReadByte()

	switch opcode {
	case packet.WARDEN_CMSG_CHEAT_CHECKS_RESULT:
		glogger.Warnln("Got cheat check.")
		s.HandleCheck(p)
	case packet.WARDEN_CMSG_MEM_CHECKS_RESULT:
		glogger.Warnln("Got mem checks")
	case packet.WARDEN_CMSG_MODULE_MISSING:
		glogger.Warnln("Client says Warden module is missing.")
		s.TransferWardenModule()
	case packet.WARDEN_CMSG_MODULE_OK:
		glogger.Warnln("Client says module is OK!")
		s.RequestHash()
	case packet.WARDEN_CMSG_HASH_RESULT:
		glogger.Warnln("Client sends hash result")
		s.HandleHashResult(p)
		s.InitializeModule()
	case packet.WARDEN_CMSG_MODULE_FAILED:
		glogger.Warnln("Fatal error: warden failed on target.")

	default:
		glogger.Warnln("Unrecognized packet:", opcode)
	}
...
func (s *Session) InitializeModule() {
	...
	s.WriteCrypt(pkt.Encode())
	glogger.Warnln("Module initialized.")
	s.RequestData()
}
...
func (s *Session) UpdateWarden() {
	...
	glogger.Warnln("Time since last update: ", diff)
	s.RequestData()
}
...
func (s *Session) RequestData() {
	...
	for _, chk := range gc {
		if chk.Type == packet.DRIVER_CHECK {
			...
			glogger.Println("Appending driver name", chk.Str)
			pkt.WriteBytes(append([]byte(chk.Str), 0))
		}
	}
	pkt.WriteByte(0x00)
	pkt.WriteByte(packet.TIMING_CHECK ^ xorByte)
	var index uint8 = 1
	glogger.Warnln("Got checks, ", len(gc))
	...
	d := pkt.Encode()
	glogger.Warnln("Encoded check packet", len(d))
	s.Warden.OutputCrypto.Encrypt(d)
	wp := packet.NewWorldPacket(packet.SMSG_WARDEN_DATA)
	wp.Write(d)
	s.WriteCrypt(wp.Encode())
